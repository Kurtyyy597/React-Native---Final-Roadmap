...
export default function FindQuiz1() {
  ...
  const deleteStudentById = (id: number) => {
    ...
    <SafeAreaView style = {[styles.container, {backgroundColor: theme.accent}]}>
      {students.map(({id, name, age}) => (
        <View key={id} style = {[styles.outputContainer, {backgroundColor: theme.surface}]}>
          <Text style = {{textAlign: 'center', fontSize: 20, fontWeight: 'bold', color: theme.textPrimary}}> {name} - {age} </Text>
          <Pressable
          onPress={()=>deleteStudentById(id)}
