...
export default function FilterQuiz2() {
  ...
  return (
    <SafeAreaView style = {[styles.container, {backgroundColor: theme.surface}]}>
      {students.map(({id, name, age}) => (
        <Text key={id} style = {{color: theme.textPrimary, fontSize: 20, fontWeight: 'bold', textAlign: 'center'}}>
          {name} - {age}
        </Text>
      ))}
        <Pressable
        onPress={findAgebetween18and25}
        ...
        styles.button,
        pressed && {opacity: 0.1 }]}>
          <Text style = {[styles.textButton, {color: theme.textPrimary}]}> filter 18 and 25 </Text>
        </Pressable>
    </SafeAreaView>
  );
};
