...
export default function FilterQuiz1() {
  const [darkMode, setDarkMode] = useState<boolean>(false);
  const theme = darkMode ? darkTheme : lightTheme;
  const originalPeople : Student[] = ([
  ...
  const [students, setStudents] = useState<Student[]>(originalPeople);

  const findNamewithLetterA = () => {
  ...
  return (
    <SafeAreaView style = {[styles.container, {backgroundColor: theme.inputBackground}]}>
      {students.map(({id, name, age}) => (
        <View key={id} style = {[styles.containerOutput, {backgroundColor: theme.surface}]}>
          <Text style = {[styles.textOutput, {color: theme.textPrimary}]}> {name} - {age} </Text>
        </View>
      ))}
       <Pressable
        onPress={findNamewithLetterA}
        ...
        styles.button,
        pressed && {opacity: 0.1 }]}>
          <Text style = {[styles.textButton, {color: theme.textPrimary}]}> Find name with A </Text>
        </Pressable>
    </SafeAreaView>
  );
};
