...
export default function DynamicRemoving() {
  ...
  const removingDynamic = (id: number) => {
      ...
      <View style = {[styles.card, {backgroundColor: theme.accent}]}>
        {items.map(({id, name}) => (
          <View key={id} style = {[styles.outputContainer, {backgroundColor: theme.accent}]}>
            <Text style = {[styles.textOutput, {color: theme.textPrimary}]}> {id}. {name} </Text>
              <Pressable
              onPress={()=>removingDynamic(id)}
