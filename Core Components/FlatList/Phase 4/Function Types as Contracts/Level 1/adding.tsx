...
import {View, Text, Pressable, Image, ActivityIndicator, StyleSheet, Alert, FlatList, ListRenderItem} from 'react-native';
  ...
  return (
    <Pressable
          ...
          isSelected && styles.selectedCard]}>
            ...
            <Pressable
              onPress={() => onAction("ADDING", student.id)}
              onLongPress={() => alert(`Long pressed: ${student.name}`)}
              delayLongPress={400}
              android_ripple={{ color: "#4818e5ff" }}
              hitSlop={10}
              ...
              styles.buttonDelete,
              pressed && {opacity: 0.1},]}>
                <Text style = {[styles.textButtonDelete, {color: theme.textPrimary}]}> Add </Text> 
            </Pressable>
          </Pressable>
  );
};

export default function FunctionTypesWithAdding() {
  ...
  const handleStudentAction: StudentControl = (action, id) => {
    switch (action) {
      case "SELECT": {
        setSelectedId(prev =>
        (prev === id ? null : id)
